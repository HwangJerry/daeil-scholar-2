...
func wireDeps(db *sqlx.DB, cfg *config.Config, logger zerolog.Logger) (*deps, error) {
	...
	return &deps{
		...
		visitJob:               visitJob,
		privacyRetentionJob:    job.NewPrivacyRetentionJob(&repository.AccountDeletionRequestRepository{DB: db, SiteOrigin: cfg.Server.SiteBaseURL, WaitHours: cfg.AccountErasure.WaitHours, TestUserSeq: cfg.AccountErasure.TestUserSeq}, logger),
