...
func TestSocialProvidersApplySameLoginEligibility(t *testing.T) {
	tests := []struct {
		...
		{name: "kakao pending", provider: model.SocialProviderKakao, status: "BBB", wantStatus: model.SocialAuthAuthenticated},
		{name: "apple pending", provider: model.SocialProviderApple, status: "BBB", wantStatus: model.SocialAuthAuthenticated},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.wantStatus == model.SocialAuthAuthenticated {
				expectAuthPrincipal(mock, test.status, model.VerificationPending)
			...
			if test.wantStatus == model.SocialAuthAuthenticated && (result.Session == nil || result.Session.User.Verification.Status != model.VerificationPending) {
				t.Fatalf("pending principal session = %#v", result.Session)
			}
...
func TestMobileSessionIssuerUsesDistinctTTLs(t *testing.T) {
	auth, mock, cleanup := newAuthServiceForTest(t)
	defer cleanup()
	expectAuthPrincipal(mock, "CCC", model.VerificationApproved)
	mock.ExpectExec(`INSERT INTO ALUMNI_MOBILE_REFRESH_TOKEN`).
		WithArgs(sqlmock.AnyArg(), 42, sqlmock.AnyArg(), sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestCompletedPendingMobileSocialLinkReturnsRestrictedSession(t *testing.T) {
	auth, mock, cleanup := newAuthServiceForTest(t)
	defer cleanup()
	expectAuthPrincipal(mock, "BBB", model.VerificationPending)
	mock.ExpectExec(`INSERT INTO ALUMNI_MOBILE_REFRESH_TOKEN`).
		WithArgs(sqlmock.AnyArg(), 42, sqlmock.AnyArg(), sqlmock.AnyArg()).
		WillReturnResult(sqlmock.NewResult(1, 1))
	social := NewSocialAuthService(
	...
	if result.Session.User.Verification.Status != model.VerificationPending || result.Session.AccessToken == "" || result.Session.RefreshToken == "" {
		t.Fatalf("restricted session = %#v", result.Session)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
	}
...
func TestRefreshReadsLatestLifecycleAndRevokesSuspendedFamily(t *testing.T) {
	auth, mock, cleanup := newAuthServiceForTest(t)
	defer cleanup()
	now := time.Now()
	...
	mock.ExpectQuery(`FROM WEO_MEMBER\s+WHERE USR_SEQ`).
		WithArgs(42).
		WillReturnRows(memberRows("DDD"))
	...
	if !errors.Is(err, ErrLoginSuspended) {
		t.Fatalf("Rotate error = %v, want suspended", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
	}
...
func TestRefreshLoadsPendingPrincipalWithoutRevokingFamily(t *testing.T) {
	auth, mock, cleanup := newAuthServiceForTest(t)
	defer cleanup()
	now := time.Now()
	...
	session, err := NewMobileSessionIssuer(auth).Rotate(token)
	if err != nil {
		t.Fatal(err)
	}
	if session.User.Verification.Status != model.VerificationPending {
		t.Fatalf("verification = %q", session.User.Verification.Status)
	}
	if session.AccessToken == "" || session.RefreshToken == "" {
		t.Fatalf("rotated session missing tokens: %#v", session)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
	}
...
func expectAuthPrincipal(mock sqlmock.Sqlmock, lifecycleStatus string, verificationStatus model.VerificationStatus) {
	mock.ExpectQuery(`LEFT JOIN ALUMNI_VERIFICATION`).
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			42, "member", "Member", lifecycleStatus, "member@example.com", nil,
			string(verificationStatus), nil, nil, nil, nil, nil, nil,
		))
}
