...
func TestKakaoIdentityVerifierValidAndInvalidAccessToken(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			mockHTTPClient := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
			...
			auth := &AuthService{
				...
				logger:      zerolog.Nop(),
				kakaoClient: kakao.NewClient(kakao.Config{HTTPClient: mockHTTPClient}),
			}
