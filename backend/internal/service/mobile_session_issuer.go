...
func (i *MobileSessionIssuer) Issue(user *model.User) (*model.MobileSession, error) {
	...
	authUser, err := i.auth.repo.GetAuthPrincipalBySeq(user.USRSeq)
	if err != nil {
		return nil, err
	}
	if authUser == nil {
		return nil, errors.New("authenticated principal not found")
	}
	now := i.now()
...
func (i *MobileSessionIssuer) Rotate(refreshToken string) (*model.MobileSession, error) {
	...
	now := i.now()
	authUser, err := i.auth.repo.GetAuthPrincipalBySeq(currentUser.USRSeq)
	if err != nil {
		return nil, err
	}
	if authUser == nil {
