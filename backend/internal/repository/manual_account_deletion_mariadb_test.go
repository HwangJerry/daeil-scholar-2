...
func TestManualAccountDeletionLifecycleOnMariaDB101(t *testing.T) {
	...
	db.MustExec(`CREATE TABLE WEO_MEMBER (USR_SEQ INT PRIMARY KEY, USR_STATUS CHAR(3)) ENGINE=InnoDB;
        ...
        CREATE TABLE ALUMNI_SOCIAL_REVOCATION_OUTBOX (
            ...
            STATUS VARCHAR(30), ATTEMPT_COUNT INT NOT NULL DEFAULT 0, NEXT_ATTEMPT_AT DATETIME, LAST_ERROR VARCHAR(500) NULL, CREATED_AT DATETIME, UPDATED_AT DATETIME) ENGINE=InnoDB;
	...
	for _, path := range []string{"../../migrations/055_create_message_reports.sql", "../../migrations/056_create_account_deletion_requests.sql", "../../migrations/057_create_automatic_account_erasure.sql", "../../migrations/059_create_erasure_context.sql", "../../migrations/060_create_erasure_targets.sql", "../../migrations/061_create_erasure_receipt_work.sql", "../../migrations/066_schedule_account_erasure.sql", "../../migrations/067_cancel_account_deletion.sql", "../../migrations/068_create_erasure_restore_guard.sql"} {
	...
	if receipt.Status != "pending" || receipt.TargetAt.Sub(receipt.RequestedAt) != 3*24*time.Hour || receipt.DueAt.Sub(receipt.TargetAt) != 10*24*time.Hour {
