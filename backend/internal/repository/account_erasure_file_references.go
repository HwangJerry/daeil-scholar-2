...
// user=0 checks all surviving accounts, after the erased member transaction.
// Locking reads cover the scanned ranges through unlink and queue acknowledgement.
func rejectOtherErasureFileReferences(tx *sqlx.Tx, s erasureSchema, local, origin string, user int) error {
	...
	for _, table := range []string{"ALUMNI_UPLOAD_OWNER", "ALUMNI_PROFILE_FILE_HISTORY"} {
		if s[table] != nil {
			queries = append(queries, referenceQuery{"SELECT URL_PATH FROM " + table + " WHERE USR_SEQ<>? FOR UPDATE", []interface{}{user}, false})
		}
	}
	for _, column := range []string{"USR_PHOTO", "USR_BIZ_CARD", "USR_THUMNAIL"} {
		if s.has("WEO_MEMBER", column) {
			queries = append(queries, referenceQuery{"SELECT COALESCE(" + column + ",'') FROM WEO_MEMBER WHERE USR_SEQ<>? FOR UPDATE", []interface{}{user}, false})
		}
	}
	for _, column := range []string{"CONTENTS", "CONTENTS_MD", "THUMBNAIL_URL", "FILES", "RE_FILES"} {
		if s.has("WEO_BOARDBBS", column) {
			queries = append(queries, referenceQuery{"SELECT COALESCE(" + column + ",'') FROM WEO_BOARDBBS WHERE USR_SEQ<>? FOR UPDATE", []interface{}{user}, true})
		}
	}
	if s["WEO_FILES"] != nil {
		predicate := "F_JOIN_SEQ<>0"
		args := []interface{}{}
		if user != 0 && s["WEO_BOARDBBS"] != nil {
			predicate += " AND NOT (F_GATE='BB' AND F_JOIN_SEQ IN (SELECT SEQ FROM WEO_BOARDBBS WHERE USR_SEQ=?))"
			args = append(args, user)
		}
		if user != 0 && s["ALUMNI_UPLOAD_OWNER"] != nil {
			predicate += " AND F_SEQ NOT IN (SELECT F_SEQ FROM ALUMNI_UPLOAD_OWNER WHERE USR_SEQ=?)"
			args = append(args, user)
		}
	...
	for _, query := range queries {
		...
		for _, value := range values {
			...
			for _, raw := range urls {
				candidate, external, err := model.ErasureFilePath(raw, origin)
				if err == nil && !external && candidate == local {
...
func (r *AccountDeletionRequestRepository) EraseFileIfUnreferenced(w model.ErasureWork, file model.ErasureFile, erase func(string) error) error {
	...
	if err = rejectOtherErasureFileReferences(tx, s, local, r.SiteOrigin, 0); err != nil {
		return err
	}
