...
func (r *AccountDeletionRequestRepository) List(status string, before int64) ([]model.AccountDeletionQueueItem, error) {
	...
	err := r.DB.Select(&items, `SELECT `+deletionReceiptColumns+`, USR_SEQ, EVIDENCE_REFERENCE,
        ...
        EXISTS(SELECT 1 FROM ALUMNI_SOCIAL_REVOCATION_OUTBOX o WHERE o.USR_SEQ=ALUMNI_ACCOUNT_DELETION_REQUEST.USR_SEQ AND o.ACTION='ACCOUNT_DELETE'
            AND (o.STATUS IN ('FAILED','FINALIZE_FAILED') OR (o.STATUS IN ('PENDING','REVOKED') AND o.CREATED_AT < DATE_SUB(UTC_TIMESTAMP(), INTERVAL 15 MINUTE)))) AS SOCIAL_UNLINK_STALLED
