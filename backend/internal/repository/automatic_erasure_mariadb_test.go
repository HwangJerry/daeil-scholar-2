...
func TestAutomaticErasureOnMariaDB101(t *testing.T) {
    ...
    CREATE TABLE ALUMNI_SOCIAL_REVOCATION_OUTBOX (ID BIGINT AUTO_INCREMENT PRIMARY KEY,USR_SEQ INT,PROVIDER CHAR(2),ACTION VARCHAR(30),STATUS VARCHAR(30),ATTEMPT_COUNT INT NOT NULL DEFAULT 0,NEXT_ATTEMPT_AT DATETIME,LAST_ERROR VARCHAR(500) NULL,CREATED_AT DATETIME,UPDATED_AT DATETIME) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
 ...
 INSERT INTO WEO_BOARDLIKE VALUES (30,42,11),(31,43,10),(32,43,11);`)
	...
	var guardRows int
	if err = db.Get(&guardRows, `SELECT COUNT(*) FROM ALUMNI_ERASURE_RESTORE_GUARD WHERE USR_SEQ=42`); err != nil || guardRows != 1 {
		t.Fatal("completed erasure not guarded against backup restore", guardRows, err)
	}
}
