...
func (h *AuthHandler) MobileLogin(w http.ResponseWriter, r *http.Request) {
	...
	if strings.TrimSpace(request.USRID) == "" || request.Password == "" {
		respondError(w, http.StatusBadRequest, "INVALID_REQUEST", "아이디와 비밀번호를 입력하세요")
		return
	}
	user, err := h.memberSvc.LoginWithPassword(request.USRID, request.Password)
	if err != nil {
...
func (h *AuthHandler) KakaoMobileLogin(w http.ResponseWriter, r *http.Request) {
	...
	writeKakaoCompatibilityResult(w, result)
}
...
func writeKakaoCompatibilityResult(w http.ResponseWriter, result model.SocialAuthResult) {
	...
	case model.SocialAuthAuthenticated:
		...
		respondJSON(w, http.StatusOK, map[string]interface{}{
			...
			"session":          session,
			"usrSeq":           session.User.USRSeq,
			"usrId":            session.User.USRID,
			"usrName":          session.User.USRName,
			"usrStatus":        session.User.USRStatus,
			"accessToken":      session.AccessToken,
			"refreshToken":     session.RefreshToken,
			"accessIssuedAt":   session.AccessIssuedAt,
	...
	case model.SocialAuthLinkRequired:
		context := result.LinkRequired
		respondJSON(w, http.StatusAccepted, map[string]interface{}{
			"status":       result.Status,
			"linkRequired": context,
			"linkToken":    context.LinkToken,
			"provider":     context.Provider,
			"email":        context.Profile.Email,
			"nickname":     context.Profile.DisplayName,
		})
