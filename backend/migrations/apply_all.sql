...
CALL _add_column_if_not_exists('ALUMNI_MOBILE_DEVICE_TOKEN', 'APNS_ENVIRONMENT', 'ENUM(''sandbox'',''production'') NOT NULL DEFAULT ''production'' AFTER DEVICE_TOKEN');
CALL _add_column_if_not_exists('ALUMNI_MOBILE_DEVICE_TOKEN', 'BUNDLE_ID', 'VARCHAR(255) NULL AFTER APNS_ENVIRONMENT');
...
CREATE TABLE IF NOT EXISTS ALUMNI_SOCIAL_REVOCATION_OUTBOX (
    ...
    INDEX IDX_SRO_CLAIM (CLAIM_TOKEN)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
CALL _add_column_if_not_exists('ALUMNI_SOCIAL_REVOCATION_OUTBOX', 'CLAIM_TOKEN', 'VARCHAR(64) NULL AFTER STATUS');
CALL _add_index_if_not_exists('ALUMNI_SOCIAL_REVOCATION_OUTBOX', 'IDX_SRO_CLAIM', 'CLAIM_TOKEN');
...
-- 034: Social revocation outbox row-claiming
SELECT 'ALUMNI_SOCIAL_REVOCATION_OUTBOX.CLAIM_TOKEN' AS chk, COUNT(*) AS found FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='ALUMNI_SOCIAL_REVOCATION_OUTBOX' AND COLUMN_NAME='CLAIM_TOKEN';
