      ...
      <div className="mx-auto grid max-w-[1080px] gap-10 px-5 py-12 sm:px-8 md:px-6 lg:grid-cols-[15rem_minmax(0,1fr)] lg:gap-16 lg:py-16">
        ...
        <article className="min-w-0" aria-label="개인정보 처리 내용">
          ...
          <PolicySection id="retention">
            ...
            <Card className="border-border bg-primary-light p-6 shadow-none">
              ...
              <p className="mt-3 text-sm leading-7 text-primary">
                앱에서 계정 삭제를 요청하면 계정 이용이 즉시 중지됩니다. 접수 후 안내되는 예약 시각 이후 자동으로 탈퇴를 처리하며 관리자가 그전에 처리를 시작할 수 있습니다. 별도 확인이 필요한 경우 담당자가 처리하고, 결과 안내 기한과 법정 보존 내역은 처리 현황에서 안내합니다. 처리가 지연되면 담당자가 사유와 예정일을 안내합니다. 앱 로그인 화면의 ‘계정 삭제 처리 현황’에서도 결과를 확인할 수 있습니다.
              </p>
            </Card>
            <p>회원정보·프로필·사진·명함·본인이 작성한 게시글·댓글·첨부파일·좋아요·쪽지와 계정에 연결된 인증정보는 이용 목적이 끝나면 지체 없이 삭제하고, 계정 삭제를 요청하면 안내된 예약 시각 이후 자동으로 삭제합니다. 실제 처리가 시작되기 전까지는 신청을 취소할 수 있습니다. 탈퇴 계정이 주고받은 쪽지는 상대방 화면에서도 삭제합니다. Apple·카카오 연결에 대한 권한 철회도 함께 처리합니다. 법령에 따라 남겨야 하는 자료는 필요한 항목만 별도로 보관하며 다른 서비스 이용에 쓰지 않습니다.</p>
            <dl className="divide-y divide-border">
              <div className="py-4"><dt className="font-semibold text-text-primary">기부금 영수증과 회계 자료</dt><dd className="mt-2 text-sm leading-7">소득세법 제160조의3 또는 법인세법 제112조의2에 따른 보관 의무가 적용되는 기부자별 발급명세는 발급일부터 5년간 보관합니다. 해피나눔에서 디지털로 발급한 영수증 원본은 해피나눔 시스템에 보관합니다. 계좌이체 기부 원본은 별도 회계 엑셀로 관리합니다. 디지털 발급만으로 홈택스 전자기부금영수증의 법정 예외를 적용하지 않습니다. 상속세 및 증여세법 제51조가 적용되는 공익법인등의 장부와 중요한 증명서류는 해당 사업연도 종료일인 12월 31일부터 10년간 보관합니다. 앱 회원 연결·전화번호·기수·학과 등 보존에 불필요한 정보는 삭제하고, 필요한 기부자명·기부일·금액·거래 증빙은 접근을 제한하여 암호화 분리 보관한 뒤 보존 기간이 끝나면 삭제합니다. 개인을 식별할 수 없는 전체 모금 집계는 유지합니다. 이 기간을 모든 기부 내역이나 회원정보에 일괄 적용하지 않으며, 장학회에 적용되는 보존 의무와 자료의 성격에 따라 항목별로 관리합니다.</dd></div>
              ...
              <div className="py-4"><dt className="font-semibold text-text-primary">방문 기록과 삭제 요청 확인 자료</dt><dd className="mt-2 text-sm leading-7">방문 상세 기록은 90일 후 정리하고 개인을 식별할 수 없는 집계만 유지합니다. 외부 삭제 작업에 필요한 식별정보는 별도 암호화하여 임시 보관하고, 외부 처리 확인 시 지체 없이 삭제합니다. 임시 작업 정보는 생성 후 10일이 지나면 사용을 중단하고 정기 정리 작업으로 삭제하며, 미완료 요청은 담당자가 확인합니다. 삭제 완료 시 접수증에서 회원 번호 연결을 제거하며, 처리 결과 조회를 위해 완료 후 30일간 확인 자료를 보관한 뒤 삭제합니다. 백업 복원 시 탈퇴한 회원의 정보를 다시 삭제하기 위해 회원 번호만 삭제 완료 후 35일간 별도로 보관한 뒤 삭제합니다.</dd></div>
            </dl>
            <p>탈퇴 시 진행 중인 기부금영수증 발급·정정·재발급 업무가 있으면, 해당 업무에 필요한 연락처는 해피나눔 또는 별도 회계 원본에서 목적에 필요한 범위로 관리합니다. 담당자는 구체적인 업무와 보존 근거를 확인하고, 업무 처리와 결과 전달이 완료되면 불필요해진 연락처를 지체 없이 정리합니다. 연락처를 동문 검색이나 후원 홍보에 사용하지 않으며, 향후 필요할 수 있다는 이유만으로 계속 보관하지 않습니다. 법령에 따라 유지해야 하는 실명·거래 증빙은 해당 보존 기준을 따릅니다.</p>
            <p>Sentry는 무료 요금제에서 새로 수집하는 오류·성능 기록을 30일간 보관합니다. 무료 요금제 전환 전 신규 계정 체험 기간에 수집한 오류 기록은 최대 90일, 성능 기록은 30일간 보관될 수 있습니다. 보관 기간은 수집 당시 기준이 적용되며 요금제 전환으로 기존 기록의 기간이 단축되지는 않습니다. Sentry의 운영 데이터 백업은 자료 종류에 따라 백업 생성 후 30일 또는 90일에 삭제됩니다. 장학회 서버는 장애 복구를 위해 회원정보와 업로드 파일을 포함한 전체 백업을 매주, 그리고 서비스를 업데이트할 때 만들며, 백업은 생성 후 28일이 지나면 삭제합니다. 가비아 자동 백업·스냅샷과 서버 이미지 백업은 사용하지 않습니다. 탈퇴한 회원의 정보는 운영 데이터베이스에서 삭제되지만, 탈퇴 전에 만든 백업에는 최대 28일 동안 남을 수 있습니다. 이 기간에 백업을 복원해야 하면 서비스를 다시 열기 전에 탈퇴한 회원의 정보를 다시 삭제합니다. 서버 접속·오류 기록도 28일 동안 보관한 뒤 삭제합니다. 계정 삭제 작업에는 백업·업로드 파일·외부 서비스의 해당 개인 데이터 확인도 포함되며, 자동으로 확인되지 않은 항목은 담당자가 확인한 뒤 완료로 처리합니다.</p>
          ...
          <PolicySection id="choices">
            ...
            <p>앱의 ‘내정보 → 계정 설정 → 계정 삭제 안내 보기’에서 안내를 확인한 뒤 별도 이메일 발송 없이 요청할 수 있습니다. 동문 인증 대기 화면에서도 삭제 요청이 가능합니다. 삭제·보존 내역에 이의가 있으면 아래 개인정보 요청 접수 이메일로 연락해 주세요.</p>
          </PolicySection>
