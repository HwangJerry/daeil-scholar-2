...
- **재사용 탐지가 어디에도 기록되지 않는다.** `auth_mobile_handler.go:71-72`는 `REFRESH_REPLAY_DETECTED`를 응답으로만 내보내고 로그를 남기지 않는다. DB에서도 정상 로그아웃과 구분되지 않는다(4-5 참고). 따라서 실제 공격이 섞여 들어와도 알아챌 수단이 현재 없다

**빈도에 대해 말할 수 있는 것과 없는 것.** 발생 빈도를 뒷받침할 데이터가 없다. 현장 보고도 없고, 위 이유로 서버 기록도 없다. 다만 구조상 **갱신은 시간에 고르게 흩어져 있지 않다**: 액세스 토큰 수명 15분(`ACCESS_TOKEN_TTL`)에 만료 120초 전 갱신(`AppConfig.refreshThresholdSeconds`)이므로, 15분 이상 앱을 닫아둔 뒤의 거의 모든 콜드 스타트가 부트스트랩 갱신을 유발한다. 그리고 그 직후는 사용자가 화면을 옮기거나 앱을 다시 닫을 확률이 가장 높은 순간이다. 갱신과 취소 위험이 같은 지점에 몰려 있으므로, "전체 시간 중 취약 구간 비율" 같은 계산은 이 상관을 0으로 놓는 셈이라 과소추정이 된다. 그런 수치는 쓰지 않는다 — 실제 값은 4-5의 측정으로만 얻을 수 있다

### 4-4. 대응 단계

| 순서 | 조치 | 비용 | 덮는 범위 | 상태 |
|---|---|---|---|---|
| 1 | **푸시 경로 지연 전송** — `onNewToken`은 로컬 저장만, 전송은 포그라운드에서 | 아주 작음 | (b)의 한 갈래 | **완료** (`8273607`) |
| 2 | **회전 구간 `NonCancellable`** — 네트워크 호출부터 `save()`까지 | 아주 작음 | (a) 전부 | **완료** (`0c7fc84`) |
| 3 | **`TokenStore.save`를 `commit()`으로** | 아주 작음 | (c) | **완료** (`0c7fc84`) |
| 4 | **매니저 스코프 분리** — `AppContainer.kt:48`의 `applicationScope`를 주입. 새 스코프를 만들지 말 것 | 중간 | 설계 부채 (아래 참고) | 보류 |
| 5 | **회전 시도 영속 표식** — 요청 직전 표식, 완료 시 제거 | 중간 | 빈도 계측, 결과 불명 판정의 입력 | **4-5 이후 결정** |
| 6 | **백엔드 유예 창** — `ROTATED_TO_JTI`로 멱등 재발급 | 중간 | (a)(b)(c)(d) 전부. 유일한 완전 해결 | **4-5 이후 결정** |
| 7 | **결과 불명 실패 처리 정책** — 갱신이 실패했는데 서버 반영 여부를 모를 때 앱이 할 일 | 중간 | 창이 열린 뒤의 동작 | **5번과 묶어서만 가능** |

**4번의 상태가 바뀌었다.** 2번을 네트워크 호출까지 포함해 감쌌기 때문에, 대표 코루틴이 갱신 도중 취소돼도 `refresh()`는 끝까지 실행되고 공유 상자는 실제 결과로 채워진다. 대기자들은 토큰을 받는다 — **무한 스피너는 이미 해소됐다.** 4번에 남은 근거는 셋뿐이며 모두 설계 부채다: `runTest` 가상 시간으로 결과 불명 시나리오를 재현할 수 있게 하는 **테스트 가능성**, 화면 해제가 네트워크 타임아웃만큼 지연되지 않게 하는 것, 공유 갱신의 **소유권 정리**. 실사용 버그가 아니므로 미뤄도 되지만, 이유는 "빈도가 낮아서"가 아니라 **"이미 해결됐기 때문"**이다.

**7번은 단독으로 설계할 수 없다.** 클라이언트는 "요청이 서버에 닿지 않음"(재시도 안전)과 "서버는 커밋, 응답만 유실"(재시도 시 세션 사망)을 **HTTP 층위에서 확실히 구분할 수 없다.** 소켓에 요청을 쓴 뒤 터널에 들어가는 경우가 정확히 그 모양이고, 이것이 모바일에서 가장 흔한 실패다. OkHttp `EventListener`로 "요청 바이트를 썼는가"까지는 알 수 있지만 **서버가 받아 커밋했는지는 알 수 없다**. 분류가 불확실한 상태에서 "결과 불명 = 로그아웃"을 넣으면, 막으려던 로그아웃보다 더 많은 로그아웃을 만들어 **순손실**이 된다. 6번이 들어온 뒤에는 재시도가 무해해지므로 이 정책 자체가 불필요해진다.

### 4-5. 빈도를 먼저 측정할 것

4~7번은 모두 "이 일이 실제로 얼마나 일어나는가"에 답이 달렸다. 지금 그 데이터가 없다.

**DB만으로는 재사용 탐지를 셀 수 없다.** 재사용 경로(`auth_repo.go:579-583`)와 정상 로그아웃(`RevokeMobileSession`)은 둘 다 같은 `MRT_SID`의 살아 있는 토큰을 같은 시각에 일괄 폐기한다. 행 상태만으로는 구분되지 않는다. 애플리케이션 로그도 없다(`auth_mobile_handler.go:71-72`는 응답만 내보낸다).

**(A) 오늘 확인할 수 있는 상한 — 시간 근접성 휴리스틱**

재사용 폐기는 직전 회전 **직후**에 일어나고, 로그아웃은 보통 한참 뒤다. MariaDB 10.1이라 CTE·윈도우 함수 없이 서브쿼리로 쓴다.

```sql
SELECT COUNT(*) FROM (
  SELECT MRT_SID,
         REVOKED_AT,
         COUNT(*)                                             AS revoked_rows,
         SUM(CASE WHEN CONSUMED_AT IS NULL THEN 1 ELSE 0 END) AS live_rows,
         MAX(CONSUMED_AT)                                     AS last_rotation
  FROM ALUMNI_MOBILE_REFRESH_TOKEN
  WHERE REVOKED_AT IS NOT NULL
  GROUP BY MRT_SID, REVOKED_AT
  HAVING revoked_rows >= 2
     AND live_rows   >= 1
     AND last_rotation IS NOT NULL
     AND TIMESTAMPDIFF(SECOND, last_rotation, REVOKED_AT) <= 5
) AS suspected_replay;

-- 분모
SELECT COUNT(DISTINCT MRT_SID) FROM ALUMNI_MOBILE_REFRESH_TOKEN;
```

5초 임계값이 판별의 핵심이다. 회전 직후의 일괄 폐기만 남기고 로그아웃을 걸러낸다. 정확한 값이 아니라 **상한**이다.

**(B) 다음 백엔드 배포에 넣을 것 — 재사용 탐지 로깅**

`auth_mobile_handler.go:71`의 재사용 분기에 구조화 로그를 한 줄 추가하면 이후로는 정확한 카운트를 얻는다. 앱 배포가 필요 없고 5번(클라이언트 영속 표식)보다 훨씬 싸다. **(B)가 있으면 5번은 불필요해질 수 있다.**

**의사결정 게이트**

| (A) 결과 | 다음 행동 |
|---|---|
| 사실상 0 | 4~7번 종료. 4번은 설계 부채로만 남긴다 |
| 유의미 | (B)를 넣어 정확히 측정하고, 6번(백엔드 유예 창) 논의로 바로 간다. 5번은 건너뛸 수 있다 |
